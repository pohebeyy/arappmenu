...
class ArActivity : AppCompatActivity() {
    ...
    private var isModelFixed = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        modelRenderer = ModelRenderer(this@ArActivity, arCore, filament)

        WindowCompat.setDecorFitsSystemWindows(window, false)
        val fixButton: Button = findViewById(R.id.fix_button)
        ...
        var isModelFixed = false
        val modelButton:Button = findViewById(R.id.model_button)


        modelButton.setOnClickListener {
            CoroutineScope(Dispatchers.Main).launch {
                try {
                    modelRenderer.loadModel(this@ArActivity, "lineika.glb")
                    Log.d("sdasd","sdsadsdasaddsa")
                    Toast.makeText(this@ArActivity, "Модель заменена", Toast.LENGTH_SHORT).show()
                } catch (e: Exception) {
                    Log.e("ArActivity", "Ошибка при замене модели: ${e.message}")
                    Log.d("sdasdddd","sdsadsdasaddsa")
                    Toast.makeText(this@ArActivity, "Не удалось загрузить модель", Toast.LENGTH_SHORT).show()
                }
            }
        }
        fixButton.setOnClickListener {
            isModelFixed = !isModelFixed // Переключаем состояние фиксации модели
            if (isModelFixed) {
                // Отключаем обработку жестов для модели
                binding.surfaceView.setOnTouchListener(null)
            } else {
                // Включаем обработку жестов для модели
                binding.surfaceView.setOnTouchListener { _, motionEvent ->
                    if (motionEvent.action == MotionEvent.ACTION_UP &&
                        (motionEvent.eventTime - motionEvent.downTime) < resources.getInteger(R.integer.tap_event_milliseconds)
                    ) {
                        Pair(
